...
{
	const cb_fn = BIND(cb, cb_data)
	const decode_fn = BIND(decode, ctx, cb_fn)
...
async function ipc_tx(ctx, op, args, force)
{
	await lock_acquire(lock)
...
export function ipc_presence(ctx, activity, force)
{
	const cmd = 'SET_ACTIVITY'
	const args = { pid, activity }
	const nonce = randomUUID()

	ipc_tx(ctx, FRAME, { cmd, args, nonce }, force)
}
