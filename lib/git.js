...
{
	if (existsSync(`${git}/rebase-merge`))
		return GIT_REBASE_MERGE
	else if (existsSync(`${git}/rebase-apply/rebasing`))
...
export function git_rebase_step_merge(git)
{
	...
	try {
		msgnum = readFileSync(`${git}/rebase-merge/msgnum`, 'utf8')
		end = readFileSync(`${git}/rebase-merge/end`, 'utf8')
	} catch {
		msgnum = 'n'
		end = 'N'
	}
...
export function git_rebase_step_apply(git)
{
	...
	try {
		next = readFileSync(`${git}/rebase-apply/next`, 'utf8')
		last = readFileSync(`${git}/rebase-apply/last`, 'utf8')
	} catch {
		next = 'n'
		last = 'N'
	}
